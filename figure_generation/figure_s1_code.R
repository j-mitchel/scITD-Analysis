...
# assign donors to groups that will be given DE multicellular processes
dg1 <- all_donors[1:20]
dg2 <- all_donors[21:40]
dg3 <- all_donors[c(1:10,21:30)]
dg4 <- all_donors[c(11:20,31:40)]

## To create a dataset with two processes, I'll combine two splatter sims together
## this is done one "cell type" at a time
# set splatter parameters
...
## now simulate a slightly bigger population of cells to add as the second factor/process
params <- newSplatParams(nGenes = 3000)
...
# bring it back to count space by multiplying each cell by half its library size from first dataset
...
########################################### now repeat this procedure for cell type 2
params <- newSplatParams(nGenes = 3000)
...
# jpeg("/home/jmitchel/figures/for_paper/sim_v3_umap_ctypes_v2.jpg")
tmp2
# dev.off()
...
##### running scITD on the simulated dataset
# set up project parameters
param_list <- initialize_params(ctypes_use = c("ct1", "ct2"),
                                ncores = 30, rand_seed = 10)

# didn't originally run with this line, but added a check for pre-normalized data. 
# To get exact results run with older version of scITD pre-oct 13 commit. Results are essentially the same.
combined_counts_sparse <- round(combined_counts_sparse) # need to make the data have integers like counts
...
pbmc_container <- determine_ranks_tucker(pbmc_container, max_ranks_test=c(7,10),
