...
#'
#' @param n_processes Number of multicellular processes to simulate
#' @param share_process_genes Set to TRUE to use the same condition DE genes all
#'        cell types (default=FALSE)
#' @param sep_ct_r_effects Set to TRUE for distinct donor random effects per cell type
...
#' @param nonlinear logical. Whether rates should be exponentiated by the effects
...
#'
#' @export
splatPopSimulate <- function(n_processes,
    ...
    sim.means <- splatPopSimulateMeans(n_processes=n_processes,
...
#'
#' @param n_processes Number of multicellular processes to simulate
#' @param share_process_genes Set to TRUE to use the same process DE genes in
#'        all cell types.
#' @param sep_ct_r_effects Set to TRUE for distinct donor random effects per cell type
#' @param vcf VariantAnnotation object containing genotypes of samples.
#' @param params SplatPopParams object containing parameters for population
...
#'
#' @export
splatPopSimulateMeans <- function(n_processes, share_process_genes, sep_ct_r_effects, vcf = mockVCF(),
                                  params = newSplatPopParams(nGenes = 1000), nonlinear=FALSE,
    ...
    for (proc_ndx in 1:n_processes) {
      conditions <- splatPopDesignConditions(params, samples) ### NEED TO DO THIS N TIMES FOR N PROCESSES
      cond_list[[proc_ndx]] <- conditions
    }
    ...
    if (!all(c("eQTL.condition", "ConditionDE.Condition1") %in% names(key))) {
      if (share_process_genes) {
        for (proc_ndx in 1:n_processes) {
          conditions <- cond_list[[proc_ndx]]
          key <- splatPopConditionEffects(params, key, conditions) # ONLY DOING THIS ONCE PER PROCESS AND COPYING EFFECTS FOR ALL CT
          colnames(key)[ncol(key)-1] <- paste0(colnames(key)[ncol(key)-1],'.',group.names[1],'.process',proc_ndx)
          colnames(key)[ncol(key)] <- paste0(colnames(key)[ncol(key)],'.',group.names[1],'.process',proc_ndx)
          for (gr_nm_ndx in 2:length(group.names)) {
            gr_nm <- group.names[gr_nm_ndx]
            key <- cbind(key,key[,c((ncol(key)-1):ncol(key))])
            colnames(key)[ncol(key)-1] <- paste0('ConditionDE.Condition2.',gr_nm,'.process',proc_ndx)
            colnames(key)[ncol(key)] <- paste0('ConditionDE.Condition1.',gr_nm,'.process',proc_ndx)
          }
        }
      } else {
        for (proc_ndx in 1:n_processes) {
          conditions <- cond_list[[proc_ndx]]
          for (gr_nm in group.names) {
            key <- splatPopConditionEffects(params, key, conditions) # NEED TO DO THIS FOR N X CT TIMES (N PROCESSES, CT CELL TYPES)
            colnames(key)[ncol(key)-1] <- paste0(colnames(key)[ncol(key)-1],'.',gr_nm,'.process',proc_ndx)
            colnames(key)[ncol(key)] <- paste0(colnames(key)[ncol(key)],'.',gr_nm,'.process',proc_ndx)
          }
        }
      }
      
      # key <- splatPopConditionEffects(params, key, conditions) # NEED TO DO THIS FOR N X CT TIMES (N PROCESSES, CT CELL TYPES)
      # colnames(key)[ncol(key)-1] <- paste0(colnames(key)[ncol(key)-1],'.Group1')
      # colnames(key)[ncol(key)] <- paste0(colnames(key)[ncol(key)],'.Group1')
      # 
      # key <- splatPopConditionEffects(params, key, conditions) # NEED TO DO THIS FOR N X CT TIMES (N PROCESSES, CT CELL TYPES)
      # colnames(key)[ncol(key)-1] <- paste0(colnames(key)[ncol(key)-1],'.Group2')
      # colnames(key)[ncol(key)] <- paste0(colnames(key)[ncol(key)],'.Group2')
    }
    ...
    if (quant.norm && ncol(means.pop) > 4){
      for (id in group.names) {
        means.pop <- splatPopQuantNorm(params, means.pop_all[[id]])
    ...
    eMeans.pop_all <- list()
    for (id in group.names) {
      eMeansPop <- splatPopSimEffects("global", key, conditions, vcf, means.pop_all[[id]])
      eMeans.pop_all[[id]] <- eMeansPop
    }
    ...
    sim.means <- splatPopSimConditionalEffects(key, eMeansPop, cond_list, nonlinear)
    })

    return(list(means = sim.means, key = key, conditions=cond_list))
}
...
splatPopSimulateSC <- function(sim.means,
    ...
    for (g in group.names) {
        ...
        for (i in seq(1, length(sims))) {
            s <- samples[i]
            sims[i][[1]]$Sample <- s
            for (proc_ndx in 1:length(conditions)) {
              c <- conditions[[proc_ndx]][s]
              sims[i][[1]][[paste0('Process',proc_ndx)]] <- c
            }
            sims[i][[1]]$Group <- g
...
#'
#' @param key Partial splatPop key data.frame.
#' @param cond_list list of condition assignments for each process
...
#' effects.
#'
splatPopSimConditionalEffects <- function(key, means.pop, cond_list, nonlinear){

  for (proc_ndx in 1:length(cond_list)) {
    conditions <- cond_list[[proc_ndx]]
    ...
    for (c in condition.list) {
      ...
      if (is.list(means.pop)) {
        for (i in names(means.pop)) { # FOR EACH CELL TYPE NEED TO USE DIFFERENT c.de (i.e., different genes effected in different cell types)
          c.de <- key[[paste0("ConditionDE.", c,'.',i,'.process',proc_ndx)]]
