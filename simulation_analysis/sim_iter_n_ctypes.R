...
get_performance_range <- function(param_test, param_range, n_iter, rand_seeds,
  ...
  cells_per_don_per_ctype <- round(cells_per_don/n_ctypes)
  ...
  for (i in 1:length(param_range)) {
    ...
    if (param_test=='n_ctypes') {
      ...
      cells_per_don <- cells_per_don_per_ctype*n_ctypes
...
r_seeds <- c(7555,15011,49323,65865,77765)

# all_res <- get_performance_range(param_test='n_ctypes', param_range=c(2,4,6,8,10), n_iter=10, 
#                                  rand_seeds=r_seeds, n_ctypes=2, ctype_de_prob=.3, n_donors=50,
#                                  cells_per_don=50, n_genes=500, n_process=2,
#                                  de_strength=1, condition.prob=c(.25,.75),
#                                  similarity.scale=1.5)
# 
# saveRDS(all_res,file='/home/jmitchel/data/sim_data/iter_n_ctypes.rds')

## trying with 1.5 de_strength 3 processes as well as 100 cells per donor 100 donors and multiple tries for Tucker
all_res <- get_performance_range(param_test='n_ctypes', param_range=c(2,4,6,8,10), n_iter=10, 
...
saveRDS(all_res,file='/home/jmitchel/data/sim_data/iter_n_ctypes_2.rds')
