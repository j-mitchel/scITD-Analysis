...
run_stability_analysis <- function(container, ranks, downsample_ratio=0.9, n_iter=500,
  ...
  # stability_results <- lapply(1:n_iter, function(i) {
    ...
    container_sub <- clean_data(container_sub, donor_min_cells=0, gene_min_cells=0)
    ...
    avmax_lds_cor <- get_max_correlations(lds_full,lds_sub,res_use='loadings')
    avmax_dnr_cor <- get_max_correlations(dnr_full,dnr_sub,res_use='dscores')
  ...
  factor_indicator <- sapply(1:n_iter,function(i) {
